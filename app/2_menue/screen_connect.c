...
void display_connect(int position);

int screen_connect(void){
    cbreak();
    noecho();
    keypad(stdscr, TRUE);
    clear();
...
void display_connect(int position) {
    ...
    if(position == 1 && flag == 1){
        ...
        mvprintw(center_y-2, center_x-10, "　　 ホストになる 　　");
    ...
    if(position == 2 && flag == 1){
        attron(COLOR_PAIR(1));
        mvprintw(center_y+1, center_x-10, "　　　　　　　　　　　");
        mvprintw(center_y+2, center_x-10, "　　　 接続する 　　　");
        mvprintw(center_y+3, center_x-10, "　　　　　　　　　　　");
        attroff(COLOR_PAIR(1));
    } else {
        attron(COLOR_PAIR(2));
